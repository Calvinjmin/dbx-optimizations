...
# MAGIC %md
# MAGIC ## 5. Dashboard
# MAGIC Open `dashboards/waf_recommendations.lvdash.json` in this Git folder —
# MAGIC Databricks renders it as a Lakeview dashboard. Attach a SQL warehouse
# MAGIC and click **Publish** the first time. The dashboard queries system
# MAGIC tables directly (same SQL as `queries/` with `waf_config` inlined as a
# MAGIC CTE), so there's no notebook output to keep in sync.
